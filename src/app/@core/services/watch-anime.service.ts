...
export class WatchAnimeService {
  ...
  setEpisode(episodeID: string, type: string, setSource: boolean = true) {
    ...
    if (type === 'client-side-vidstreaming') {
      this.getAnimeEpisodeExternalAPI(
        episodeID,
        `https://lite-api.animemate.xyz/Episode/`,
        `vidstreaming`,
        setSource
      );
  ...
  getAnimeEpisodeExternalAPI(
    ...
    if (proxy_list.length > 0) {
      externalPayloadURL = `${
        proxy_list[Math.floor(Math.random() * proxy_list.length)].url
      }${externalAPI_URL}${_title}/${_episodeNumber}`;
    } else {
